...
export default function MousePage() {
  ...
  useEffect(() => {
    async function cargar() {
      try {
        setLoading(true);
        const res = await api.get("/apij/productos/categoria/nombre/mouse");
        const rows = Array.isArray(res.data) ? res.data : (res.data.rows || []);
        setProductos(shuffleArray(rows));
      } catch (err) {
        console.error("Error cargando mouse por categoría:", err);
        setProductos([]);
      } finally {
        setLoading(false);
      }
  ...
  const renderProduct = (p) => {
    ...
    const category = (p.categoria || "mouse").toString().toLowerCase();
      ...
      <main className="w-full mx-0 px-4 sm:px-4 lg:px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <section className="flex-1 order-2">
            <div className="mb-8">
              <h1 className="text-3xl font-bold text-gray-900 mb-3">Mouse</h1>
              <p className="text-gray-600">Encuentra mouse y periféricos para tu setup</p>
            </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-16 bg-white rounded-lg shadow-sm">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">No hay mouse disponibles</h3>
              ...
              <>
                ...
                <div className="flex items-center justify-center gap-2 mt-8">
                  ...
                  {Array.from({ length: pageCount }).map((_, idx) => {
                    const pNum = idx + 1;
                    return (
                      <button key={pNum} onClick={() => setPage(pNum)} className={`px-3 py-1 rounded ${pNum === page ? 'bg-blue-600 text-white' : 'bg-gray-100'}`}>{pNum}</button>
                    );
                  })}
