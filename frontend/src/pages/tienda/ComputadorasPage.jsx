          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-2 sm:mt-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <div className="flex w-full sm:w-auto gap-2 sm:gap-3">
                ...
                <button onClick={() => handleAddToCart(p)} disabled={!p.stock || p.stock <= 0 || p.estado === 'agotado'} className={`flex-1 sm:inline-block px-3 py-2 rounded-lg text-sm font-medium transition-colors ${!p.stock || p.stock <= 0 || p.estado === 'agotado' ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>{!p.stock || p.stock <= 0 || p.estado === 'agotado' ? 'Agotado' : 'Añadir'}</button>
              </div>
            </div>
          </div>
        </article>
      ...
      <article key={p.id_producto || p.id || p.codigo} className={styles.cardClass}>
        ...
        <div className={styles.contentClass}>
          ...
          <div className="flex items-center justify-between mt-auto">
            ...
            <button onClick={() => handleAddToCart(p)} disabled={!p.stock || p.stock <= 0 || p.estado === 'agotado'} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${!p.stock || p.stock <= 0 || p.estado === 'agotado' ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>{!p.stock || p.stock <= 0 || p.estado === 'agotado' ? 'Agotado' : 'Añadir'}</button>
          </div>
        </div>
      </article>
    );
          ...
          <section className="flex-1 order-2">
            ...
            {loading ? (
              ...
              <div className="text-center py-16 bg-white rounded-lg shadow-sm">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">No hay computadoras disponibles</h3>
