...
export default function ComponentesPage() {
  ...
  useEffect(() => {
    async function cargar() {
      try {
        setLoading(true);
        const res = await api.get("/apij/productos/categoria/nombre/componentes");
        const rows = Array.isArray(res.data) ? res.data : (res.data.rows || []);
        setProductos(shuffleArray(rows));
      } catch (err) {
        console.error("Error cargando componentes por categoría:", err);
        setProductos([]);
      } finally {
        setLoading(false);
      }
  ...
  const renderProduct = (p) => {
    ...
    const category = (p.categoria || "componentes").toString().toLowerCase();
      ...
      <main className="w-full mx-0 px-4 sm:px-4 lg:px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <section className="flex-1 order-2">
            <div className="mb-8">
              <h1 className="text-3xl font-bold text-gray-900 mb-3">Componentes</h1>
              <p className="text-gray-600">Encuentra componentes para tu setup</p>
            </div>
