...
const itemsPerPage = 20;
  ...
  useEffect(() => {
    async function cargar() {
      try {
        setLoading(true);
        const res = await api.get("/apij/productos/categoria/nombre/licencia");
  ...
  const renderProduct = (p) => {
    ...
    const category = (p.categoria || "licencia").toString().toLowerCase();
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mb-3 flex items-center">
              {p.stock > 0 && p.estado !== 'agotado' ? (
                <span className="inline-flex items-center px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full font-medium">Stock: {p.stock} {p.stock === 1 ? 'unidad' : 'unidades'}</span>
              ) : (
                <span className="inline-flex items-center px-2 py-1 bg-red-100 text-red-800 text-xs rounded-full font-bold">AGOTADO</span>
              )}
              {badgeToShow}
            </div>
            ...
            <div className="mt-2 sm:mt-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <div className="flex w-full sm:w-auto gap-2 sm:gap-3">
                ...
                <button onClick={() => handleAddToCart(p)} disabled={!p.stock || p.stock <= 0 || p.estado === 'agotado'} className={`flex-1 sm:inline-block px-3 py-2 rounded-lg text-sm font-medium transition-colors ${!p.stock || p.stock <= 0 || p.estado === 'agotado' ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`} aria-label={`Añadir ${p.nombre_producto} al carrito`}>
      ...
      <article key={p.id_producto || p.id || p.codigo} className={styles.cardClass}>
        ...
        <div className={styles.contentClass}>
          ...
          <div className="flex items-center justify-between mt-auto">
            ...
            <button onClick={() => handleAddToCart(p)} disabled={!p.stock || p.stock <= 0 || p.estado === 'agotado'} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${!p.stock || p.stock <= 0 || p.estado === 'agotado' ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`} aria-label={`Añadir ${p.nombre_producto} al carrito`}>
              {!p.stock || p.stock <= 0 || p.estado === 'agotado' ? 'Agotado' : 'Añadir'}
            </button>
          </div>
        </div>
      ...
      <main className="w-full mx-0 px-4 sm:px-4 lg:px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <section className="flex-1 order-2">
            <div className="mb-8">
              <h1 className="text-3xl font-bold text-gray-900 mb-3">Licencias</h1>
              <p className="text-gray-600">Encuentra licencias para tu software</p>
            </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-16 bg-white rounded-lg shadow-sm">
                <h3 className="text-lg font-medium text-gray-900 mb-2">No hay licencias disponibles</h3>
