      ...
      <div className="w-full max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden mb-4">
          <div className="flex items-center justify-between px-4 py-3 sm:px-6">
            <div className="text-sm sm:text-base font-semibold text-blue-700">Listar Administradores</div>
            <div className="flex items-center gap-2">
              <div className="hidden sm:block text-gray-700 font-semibold">{filtered.length} usuarios</div>
              <button
                onClick={loadAdmins}
        ...
        {/* Filters */}
        <div className="mb-4">
          <div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm flex flex-col sm:flex-row gap-3 sm:items-center">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 flex-1">
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Filtrar por Rol</label>
                <select
                  value={roleFilter}
              ...
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Filtrar por Estado</label>
                <select
                  value={stateFilter}
            ...
            <div className="flex items-center gap-3 sm:ml-4">
              <div className="text-sm text-gray-500 hidden sm:block">{filtered.length} resultados</div>
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-100">
              <tr>
                ...
                <th className="text-left px-6 py-3 font-medium text-gray-700">USUARIO</th>
                ...
                <th className="text-left px-6 py-3 font-medium text-gray-700">ESTADO</th>
                    ...
                    <td className="px-6 py-4 text-sm">
                      <span
                        className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${
                          String(a.estado) === "activo" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"
                        }`}
                      >
                        {a.estado}
                      </span>
        ...
        <div className="md:hidden space-y-3">
          ...
          {!loading && filtered.length === 0 && <div className="p-6 bg-white rounded-xl text-center text-gray-500">No hay administradores</div>}

          {!loading &&
            filtered.map((a) => (
              <div key={a.id_admin || a.id} className="bg-white rounded-xl shadow p-4">
                <div className="flex items-start gap-3">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-r from-blue-500 to-blue-600 flex items-center justify-center text-white font-semibold text-lg">
                    {(a.nombre || "U").slice(0,1).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <div className="text-sm font-semibold text-gray-900 truncate">{a.nombre}</div>
                      <div className="text-xs text-gray-500">{a.usuario}</div>
                    </div>
                    <div className="mt-1 text-xs text-gray-600 truncate">{a.email || "-"}</div>
                    <div className="mt-3 flex items-center gap-2">
                      <span className="inline-block px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 text-xs font-semibold">{a.rol}</span>
                      <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold ${String(a.estado) === "activo" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>{a.estado}</span>
                    </div>

                    <div className="mt-3 flex items-center gap-2">
                      <button onClick={() => openChangePassword(a)} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-gray-100 rounded-lg text-sm">
                        <FiKey /> Contraseña
                      </button>
                      <button onClick={() => openChangeRole(a)} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-gray-100 rounded-lg text-sm">
                        <FiEdit2 /> Rol
                      </button>
                      <button onClick={() => toggleEstado(a)} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-red-50 rounded-lg text-sm text-red-600">
                        <FiPower /> {a.estado === "activo" ? "Inactivar" : "Activar"}
                      </button>
                    </div>
                  </div>
                </div>
